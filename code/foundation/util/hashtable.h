...
template<class KEYTYPE, class VALUETYPE>
bool
HashTable<KEYTYPE, VALUETYPE>::Find(const KEYTYPE& key, const VALUETYPE& outValue)const
{
	// get hash code from key, trim to capacity
	IndexT hashIndex = key.HashCode() % this->hashArray.Size();
	const Array<KeyValuePair<KEYTYPE, VALUETYPE> >& hashElements = this->hashArray[hashIndex];
	...
	else
	{
		...
		{
			outValue = hashElements[hashElementIndex].Value();
			return true;
		}
		else
			return false;
...
template<class KEYTYPE, class VALUETYPE> 
Array<VALUETYPE>
HashTable<KEYTYPE, VALUETYPE>::ContentValue()
{
	...
	for (i = 0; i < num; i++)
	{
		if (this->hashArray[i].Size() > 0)
		{
			for (SizeT j = 0; j < this->hashArray[i].Size(); j++)
			{
				result.Append(this->hashArray[i].At(j).Value());
			}
		}
	}
	return result;
