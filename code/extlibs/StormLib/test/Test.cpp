...
static int TestCreateArchiveV2(const char * szMpqCopyName, const char * szFileName)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        if(!SFileCreateArchiveEx(szMpqCopyName,
                                 MPQ_CREATE_ARCHIVE_V2 | OPEN_EXISTING,
...
//      nError = TestBlockCompress("E:\\TestFile.txt");
    ...
    if(nError == ERROR_SUCCESS)
        nError = TestArchiveOpenAndClose(szMpqName, "enGB\\Sound\\Creature\\LadySacrolash\\SACROLASHCOMBINE.wav");
...
//  if(nError == ERROR_SUCCESS)
//      nError = TestCreateArchiveV2(szMpqCopyName, "E:\\Install\\Winnt60\\vista_5728.16387.060917-1430-LRMWDK_EN.iso");
