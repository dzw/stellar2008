...
#include "coregraphics/legacy/m2streamreader.h"
...
StreamMeshLoader::SetupMeshFromStream(const Ptr<Stream>& stream)
{
	...
	else if (this->resource->GetResourceId().Value().GetFileExtension() == "m2")
	{
...
*/
bool
StreamMeshLoader::SetupMeshFromM2(const Ptr<Stream>& stream)
{
	n_assert(stream.isvalid());    
