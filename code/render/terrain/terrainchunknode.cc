...
namespace Terrain
{
ImplementClass(Terrain::TerrainChunkNode, 'CHKN', Models::TransformNode);
...
TerrainChunkNode::CreateNodeInstance() const
{
...
*/
void
TerrainChunkNode::SaveToAttrs(AttributeContainer& attrs)
{
...
*/
void
TerrainChunkNode::LoadResources()
{
	...
	n_assert(this->managedMesh.isvalid());
	DebugView::Instance()->Stop("TerrainChunkNode load", true);
...
*/
void
TerrainChunkNode::UnloadResources()
{
	n_assert(this->managedMesh.isvalid());
...
*/
void
TerrainChunkNode::ApplySharedState()
{
	n_assert(this->managedMesh.isvalid());
	n_assert(this->primGroupIndex != InvalidIndex);
