...
namespace PhysX
{
ImplementClass(PhysX::CharControlManager, 'PCCM', Core::RefCounted);
ImplementSingleton(PhysX::CharControlManager);
...
CharControlManager::CharControlManager()
{
	ConstructSingleton;
}
...
void 
CharControlManager::Open()
{
	PhysXServer* physXServer = PhysXServer::Instance();
	CreateControllerManager((NxUserAllocator*)physXServer->GetMemoryAllocator());
	this->isOpen = true;
}

void 
CharControlManager::Close()
{
...
void 
CharControlManager::CreateControllerManager(NxUserAllocator* a)
{
	ccManager = NxCreateControllerManager(a);
}

void 
CharControlManager::ReleaseControllerManager()
{
	NxReleaseControllerManager(ccManager);
}

void 
CharControlManager::UpdateControllers()
{
	ccManager->updateControllers();
}

NxController* 
CharControlManager::InitCharacterControllers(NxU32 nbCharacters, const NxVec3* startPos, NxReal scale, NxScene& scene)
{
...
void 
CharControlManager::ReleaseCharacterControllers(NxScene& scene)
{
...
#endif
NxU32 
CharControlManager::MoveCharacter(NxU32 characterIndex, NxScene& scene, const NxVec3& dispVector, NxF32 elapsedTime, NxU32 collisionGroups, NxF32 heightDelta)
{
	gNbPts = 0;
...
CharControlManager::GetCharacterPos(NxU32 characterIndex)
{
	return ccManager->getController(characterIndex)->getFilteredPosition();
}

NxActor* 
CharControlManager::GetCharacterActor(NxU32 characterIndex)
{
	return ccManager->getController(characterIndex)->getActor();
}

bool 
CharControlManager::ResetCharacterPos(NxU32 index, const NxVec3& pos)
{
	return ccManager->getController(index)->setPosition(NxExtendedVec3(pos.x, pos.y, pos.z));
}

bool 
CharControlManager::UpdateCharacterExtents(NxU32 index, bool& increase)
{
...
void 
CharControlManager::RenderCharacters()
{
