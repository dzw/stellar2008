...
SharedResourceServer::Close()
{
	...
	Array<Ptr<Resource>> res = this->resources.GetContents();
...
SharedResourceServer::HasSharedResource(const ResourceId& resId) const
{
    ...
    return this->resources.Contains(resId.Value());
}
...
{
    return this->resources[resId.Value()];
}
...
SharedResourceServer::RegisterSharedResource(const Ptr<Resource>& res)
{
    ...
    n_assert(!this->resources.Contains(res->GetResourceId().Value()));
    res->IncrUseCount();
    this->resources.Add(res->GetResourceId().Value(), res);
}
...
SharedResourceServer::UnregisterSharedResource(const Ptr<Resource>& res)
{
    ...
    if (res->GetUseCount() == 0)
    {
        ...
        res->SetLoader(0);
        res->SetSaver(0);
        this->resources.Erase(res->GetResourceId().Value());
    }
}
...
SharedResourceServer::UnregisterSharedResource(const ResourceId& id)
{
    ...
    Ptr<Resource> res = this->resources[id.Value()];
...
{
    n_assert(resId.Value().IsValid());
    if (this->resources.Contains(resId.Value()))
    {
        // return existing resource
        const Ptr<Resource>& res = this->resources[resId.Value()];
...
SharedResourceServer::GetSharedResourcesByType(const Core::Rtti& type) /*const*/
{
	...
	/*const*/ Ptr<Resource>& res = this->resources.GetFirstValue();
	while (res.isvalid())
	{
		if (res->IsA(type))
		{
		...
		res = this->resources.GetNextValue();
	}
