...
Stage::UpdateEntities(Timing::Time curTime, IndexT curFrameCount)
{
    ...
    if (curFrameCount != this->updateEntitiesFrameCount)
    {
     //   Timing::Timer tt;
        ...
        CoreGraphics::DebugView::Instance()->AddDebugString("UpdateEntities", output);*/
    }
}
...
Stage::UpdateCameraLinks(const Ptr<CameraEntity>& cameraEntity)
{
	...
	String output;
	output.AppendFloat(t);
	CoreGraphics::DebugView::Instance()->AddDebugString("UpdateCameraLinks", output);
}
