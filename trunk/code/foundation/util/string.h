...
public:
    ...
    void operator +=(const String& rhs);
	///
	int FastCompare(const String& b)const; 
	int FastCompare(const char* cStr)const;
	//friend int FastCompare(const char* cStr, const String& b);
    /// equality operator
...
*/
inline int 
String::FastCompare(const String& b)const
{
	if (this->AsCharPtr() == b.AsCharPtr())
		return 0;

	if (this->strLen == b.Length())
		return memcmp(this->AsCharPtr(), b.AsCharPtr(), this->strLen);

	return this->strLen - b.Length();
}
...
*/
inline int 
String::FastCompare(const char* cStr)const
{
	if (this->AsCharPtr() == cStr)
		return 0;

	SizeT len = (SizeT)strlen(cStr);
	if (this->strLen == len)
		return memcmp(this->AsCharPtr(), cStr, this->strLen);

	return this->strLen - len;
}
...
{
    n_assert(0 != cStr);
	return a.FastCompare(cStr) == 0;

	if ((a.Length() - strlen(cStr)) != 0)
		return false;
...
{
    n_assert(0 != cStr);
	return b.FastCompare(cStr) == 0;

	if ((strlen(cStr) - b.Length()) != 0)
		return false;
...
{
public:
	SizeT operator()(const String& _Keyval)const
	{
		return _Keyval.HashCode();
	}
	bool operator()(const String& l, const String& r)const
	{
		return l.FastCompare(r) < 0;
	}
};

//struct StringCompare 
//{
//	SizeT operator()(const String& l, const String& r)const
//	{
//		return l.FastCompare(r);
//	}
//};
//inline SizeT hash_value(const String& key)
//{
//	return key.HashCode();
//}
