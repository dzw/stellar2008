...
unsigned int endHighResolutionTimer(unsigned int time[2])
{
   unsigned int ticks;
...
Profiler::Profiler()
{
   ...
   for(unsigned int i = 0; i < ProfilerData::HashTableSize; i++)
...
const char * Profiler::constructProfilePath(ProfilerData * pd)
{
   if (pd->mParent)
   {
      ...
      unsigned int len = dStrlen(pd->mParent->mPath);
      if (!len)
         connector = "";
      len += dStrlen(connector);
      len += dStrlen(pd->mRoot->mName);
      unsigned int mark = FrameAllocator::getWaterMark();
...
void Profiler::hashPush(ProfilerRootData *root)
{
   ...
   mStackDepth++;
   n_assert2((SizeT)mStackDepth <= mMaxStackDepth,
   ...
   if(!nextProfiler)
   {
      ...
      unsigned int index = root->mNameHash & (ProfilerData::HashTableSize - 1);
...
{
   n_assert2(strlen(fileName) < DumpFileNameLength, "Error, dump filename too long");
   mDumpToFile = true;
   mDumpToConsole = false;
   strcpy(mDumpFileName, fileName);
}
...
void Profiler::hashPop()
{
   ...
   mStackDepth--;
   n_assert2(mStackDepth >= 0, "Stack underflow in profiler.  You may have mismatched PROFILE_START and PROFILE_ENDs");
   if(mEnabled)
   {
      ...
      double fElapsed = endHighResolutionTimer(mCurrentProfilerData->mStartTime);
...
static signed int /*QSORT_CALLBACK*/ rootDataCompare(const void *s1, const void *s2)
{
   const ProfilerRootData *r1 = *((ProfilerRootData **) s1);
   const ProfilerRootData *r2 = *((ProfilerRootData **) s2);
   return (signed int)((r2->mTotalTime - r2->mSubTime) - (r1->mTotalTime - r1->mSubTime));
}

static void profilerDataDumpRecurse(ProfilerData *data, char *buffer, unsigned int bufferLen, double totalTime)
{
   // dump out this one:
	String out;
	out.Format("%7.3f %7.3f %8d %s%s\n",
...
//static void profilerDataDumpRecurseFile(ProfilerData *data, char *buffer, unsigned int bufferLen, double totalTime, FileStream& fws)
//{
//   char pbuffer[256];
//   _sprintf(pbuffer, 255, "%7.3f %7.3f %8d %s%s\n",
//         100 * data->mTotalTime / totalTime,
//         100 * (data->mTotalTime - data->mSubTime) / totalTime,
//         data->mInvokeCount,
//         buffer,
...
void Profiler::dump()
{
   ...
   Array<ProfilerRootData *> rootVector;
   double totalTime = 0;
   for(ProfilerRootData *walk = ProfilerRootData::sRootList; walk; walk = walk->mNextRoot)
   {
      totalTime += walk->mTotalTime - walk->mSubTime;
      rootVector.InsertSorted(walk);
   }
   ...
   {
      Core::SysFunc::DebugOut("Profiler Data Dump:\n");
      Core::SysFunc::DebugOut("Ordered by non-sub total time -\n");
      Core::SysFunc::DebugOut("%%NSTime  %% Time  Invoke #  Name\n");
      for(unsigned int i = 0; i < rootVector.Size(); i++)
      {
	     String out;

         out.Format("%7.3f %7.3f %8d %s\n",
      ...
      Core::SysFunc::DebugOut("Ordered by stack trace total time -\n");
      Core::SysFunc::DebugOut("%% Time  %% NSTime  Invoke #  Name\n");
   ...
   else if (mDumpToFile == true && mDumpFileName[0] != '\0')
   {
      ...
      //n_assert2(success, "Nuts! Cannot write profile dump to specified file!");
      //   char buffer[1024];
...
{
   reset();
   unsigned int markerLen = (unsigned int)strlen(marker);
   if(markerLen == 0)
      return;
   ...
   for(ProfilerRootData *data = ProfilerRootData::sRootList; data; data = data->mNextRoot)
   {
      if(sn)
      {
         if(!n_stricmp(marker, data->mName/*, markerLen - 1*/))
            data->mEnabled = enable;
      }
      else
      {
         if(!n_stricmp(marker, data->mName))
            data->mEnabled = enable;
      }
   }
}
