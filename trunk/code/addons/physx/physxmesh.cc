...
namespace PhysX
{
ImplementClass(PhysX::PhysXMesh, 'PYMS', Core::RefCounted);
...
PhysXMesh::Load()
{
    ...
    {
        n_error("Physics::PhysXMesh::Load(): invalid file extension in filename '%s'!", this->filename.AsCharPtr());
    }

    // load mesh data    
    if (!meshLoader->Open())
    {
        n_error("Physics::PhysXMesh:Load()(): Failed to open mesh file '%s'!", this->filename.AsCharPtr());
        return false;
    }
...
*/
void
PhysXMesh::BeginAppendMesh(int numVertices, int numIndices, int vertexWidth)
{
    n_assert(!this->isLoaded);
...
*/
bool
PhysXMesh::AppendMesh(const Ptr<PhysXMesh>& mesh, const Math::matrix44& transform)
{
...
*/
int
PhysXMesh::GetGroupNumVertices(int groupIndex) const
{
    n_assert(this->isLoaded);
...
*/
int
PhysXMesh::GetGroupNumIndices(int groupIndex) const
{
    n_assert(this->isLoaded);
...
*/
float*
PhysXMesh::GetGroupVertexPointer(int groupIndex) const
{
    n_assert(this->isLoaded);
...
*/
int*
PhysXMesh::GetGroupIndexPointer(int groupIndex) const
{
    n_assert(this->isLoaded);
...
*/
void
PhysXMesh::UpdateGroupBoundingBoxes(float* vertexBufferData, int* indexBufferData)
{
    bbox groupBox;
