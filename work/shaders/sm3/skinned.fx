...
struct vsSamplePassInputSkinned
{
    float4 position : POSITION;
    float3 normal   : NORMAL;
    float4 weights  : BLENDWEIGHT;
...
ColorPassVertexShaderSkinned(const vsColorPassInputSkinned vsIn)
{
    ...
    vsOut.tangent     = float3(1.0f,1.0f,1.0f);//normalize(mul(vsIn.tangent, model));
    vsOut.binormal    = float3(1.0f,1.0f,1.0f);//normalize(mul(vsIn.binormal, model));
    vsOut.uv0 = vsIn.uv0;
    return vsOut;
}
...
ColorPassPixelShader(const vsColorPassOutput psIn, uniform int numLights) : COLOR
{
    ...
    //float3 lightVec = lightPos[0] - psIn.worldPos;
    //color = matDiffuse * max(0,dot(psIn.normal, lightVec));
    //color.a = matDiffuse.a;
    return matDiffuse;
}
